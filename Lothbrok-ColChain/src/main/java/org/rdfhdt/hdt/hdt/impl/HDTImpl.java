...
public class HDTImpl implements HDTPrivate {
	...
	public void mapFromHDT(File f, long offset, ProgressListener listener) throws IOException {
		...
		ci.load(input);
		String hdtFormat = ci.getFormat();
		if(!hdtFormat.equals(HDTVocabulary.HDT_CONTAINER) && !hdtFormat.equals(HDTVocabulary.HDT_CONTAINER1)) {
			throw new IllegalFormatException("This software (v" + HDTVersion.HDT_VERSION + ".x.x) cannot open this version of HDT File (" + hdtFormat + ")");
		}
...
//				System.out.println("The .hdt.index doesn't exist, generating a new one.");
	...
	@Override
	public IteratorStarString searchStar(StarString star) {
		if(isClosed) {
			throw new IllegalStateException("Cannot search an already closed HDT");
		}

		IteratorStarString iterator = new DictionaryTranslateIteratorStar(star, this);
	...
	@Override
	public IteratorStarString searchStarBindings(StarString star, List<Binding> bindings) {
		if(isClosed) {
			throw new IllegalStateException("Cannot search an already closed HDT");
		}

		IteratorStarString iterator = new DictionaryTranslateIteratorStar(star, bindings, this);
