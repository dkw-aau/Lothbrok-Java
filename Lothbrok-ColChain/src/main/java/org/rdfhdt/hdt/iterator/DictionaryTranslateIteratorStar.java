...
public class DictionaryTranslateIteratorStar implements IteratorStarString {
    ...
    public DictionaryTranslateIteratorStar(StarString star, List<Binding> bindings, HDT hdt) {
        ...
        if(this.curr == null) this.currentIterator = null;
        else
    ...
    private void buffer() {
        if(this.currentIterator == null) return;
        if (currentIterator.hasNext()) {
            StarID id = currentIterator.next();
            if(id != null) {
                next = id.toStarString(hdt.getDictionary());
                return;
            }
        }
    ...
    private static class StarStringIterator implements Iterator<StarString> {
        ...
        private void bufferNext() {
            ...
            if ((bindings.size() == 0 || bindings.get(0).isEmpty()) && current == 0) {
                next = star;
                current++;
                return;
            }
            ...
            while (vars.hasNext()) {
                ...
                String val = "";
                if (node.isLiteral())
                    val = "\"" + node.getLiteral().getLexicalForm() + "\"";
                else if (node.isURI())
                    val = node.getURI();
