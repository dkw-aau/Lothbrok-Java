...
import org.lothbrok.index.index.IndexBase;
...
public class PpbfIndex extends IndexBase {
    ...
    public void updateIndex(String fragmentId, IBloomFilter<String> filter) {
        ...
        for (Tuple<IGraph, IGraph> g : kks) {
            if (g.x.getId().equals(fragmentId) || g.y.getId().equals(fragmentId)) {
    ...
    public void removeCommunity(String id) {
        ...
        for (Tuple<IGraph, IGraph> k : ks) {
            if (k.x.isCommunity(id) || k.y.isCommunity(id)) {
                ...
                for (IGraph f1 : f1s) {
                    for (IGraph f2 : f2s) {
                        ...
                        if (blooms.containsKey(tuple)) {
                            ...
                            String filename = f1.getId() + "-" + f2.getId() + ".ppbf";
                            IBloomFilter<String> intersection = bs.get(f1).intersect(bs.get(f2), filename);
