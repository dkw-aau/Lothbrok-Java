...
import org.lothbrok.index.ppbf.hash.IHashFunction;
import org.lothbrok.index.ppbf.hash.Murmur3;
...
public class PrefixPartitionedBloomFilter implements IBloomFilter<String> {
    ...
    private long estimatedCardinality = 0;
    ...
    private PrefixPartitionedBloomFilter(int numBits, int numHash, Map<String, Long> partitions, String file) {
        ...
        this.filename = file;
        currentOffset = 16 + (partitions.size() * numBits);
        numInsertedElements = estimatedCardinality();
    }
    ...
    public long estimatedCardinality() {
        if(numInsertedElements != 0) return numInsertedElements;
        long count = 0;

        for (String part : partitions.keySet()) {
            long offset = partitions.get(part);
            long bits;
            try {
                bits = numSetBits(offset);
            } catch (IOException e) {
                continue;
            }

            double c1 = Math.log(1.0-((double) bits / (double) numBits));
            double c2 = Math.log(1.0-(1.0 / (double) numBits));
            double cnt = c1/(numHashFunctions * c2);

            count += (long) cnt;
        }

        numInsertedElements = count;
        return count;
    }
    ...
    public static PrefixPartitionedBloomFilter create(double maxFpp, String file) {
        return new PrefixPartitionedBloomFilter(500000, maxFpp, file);
    }

    public static PrefixPartitionedBloomFilter create(String file) {
        return new PrefixPartitionedBloomFilter(500000, 0.1, file);
    }
    ...
    private String getPrefix(String uri) {
        ...
        if (!uri.matches(regex)) return "constants";
        if (uri.endsWith("/")) {
    ...
    public IBloomFilter<String> intersect(IBloomFilter<String> other, String filename) {
        ...
        String dirname = AbstractNode.getState().getDatastore() + "index/intersections/";
        ...
        try {
            ...
            RandomAccessFile raf1 = new RandomAccessFile(this.filename, "r");
    ...
    private int numSetBits(long offset) throws IOException {
        int num = 0;
        RandomAccessFile raf = new RandomAccessFile(filename, "r");
        for (long off = offset; off < (numBits / 8); off++) {
            raf.seek(off);
            byte b = raf.readByte();
            if(b > 0)
                num += bitCount(b);
