...
public class OpExecutorLothbrok extends OpExecutor {
    ...
    public final static OpExecutorFactory opExecFactoryLothbrok = new OpExecutorFactory() {
        @Override
    ...
    protected QueryIterator execute(OpFilter opFilter, QueryIterator input) {
        ...
        if (OpBGP.isBGP(opFilter.getSubOp())) {
            // Still may be a Colchain graph in a non-colchain dataset (e.g. a named model)
    ...
    /**
     * Execute a BGP (and filters) on a Colchain graph, which may be in default storage or it may be a named graph
     */
    ...
    private static OpExecutorFactory plainFactory = new OpExecutorPlainFactoryColchain();

    private static class OpExecutorPlainFactoryColchain implements OpExecutorFactory {
        @Override
        public OpExecutor create(ExecutionContext execCxt) {
            return new OpExecutorColchainBlock(execCxt);
        }
    }
     ...
     */
    private static class OpExecutorColchainBlock extends OpExecutor {

        @SuppressWarnings("unchecked")
        public OpExecutorColchainBlock(ExecutionContext execCxt) {
            super(execCxt);
        }
