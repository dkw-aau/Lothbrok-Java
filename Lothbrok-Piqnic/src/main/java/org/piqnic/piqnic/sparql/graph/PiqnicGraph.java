...
import org.piqnic.piqnic.sparql.PiqnicBindings;
import org.piqnic.piqnic.sparql.PiqnicJenaConstants;
import org.piqnic.piqnic.sparql.iter.PiqnicJenaIterator;
import org.piqnic.piqnic.sparql.iter.EmptyIterator;
import org.piqnic.piqnic.sparql.iter.LocalPiqnicIterator;
import org.piqnic.piqnic.sparql.iter.RemotePiqnicIterator;
import org.piqnic.piqnic.sparql.solver.PiqnicEngine;
import org.piqnic.piqnic.sparql.solver.OpExecutorPiqnic;
import org.piqnic.piqnic.sparql.solver.ReorderTransformationPiqnic;
...
public class PiqnicGraph extends GraphBase {
    ...
    public ExtendedIterator<Pair<Triple, Binding>> graphBaseFind(Triple jenaTriple, PiqnicBindings bindings, Set<IGraph> fragments) {
        Queue<NiceIterator<Pair<Triple, Binding>>> queue = new LinkedList<>();
        for (IGraph graph : fragments) {
            if (AbstractNode.getState().getDatasourceIds().contains(graph.getId())) {
                HDT datasource;
                datasource = AbstractNode.getState().getDatasource(graph.getId()).getHdt();
                queue.add(new LocalPiqnicIterator(jenaTriple, bindings, datasource));
            } else {
                String url = "";
                try {
                ...
                queue.add(new RemotePiqnicIterator(url, jenaTriple, bindings));
            }
        }

        return new PiqnicJenaIterator(queue);
    }

    private String getFragmentUrl(Triple jenaTriple, PiqnicBindings bindings, IGraph graph) throws EncoderException {
    ...
    private void appendBindings(Triple triple, StringBuilder sb, PiqnicBindings bindings) throws EncoderException {
